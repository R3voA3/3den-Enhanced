...
def getDefinedStrings(filepath):
		...
		with open(filepath, 'r', encoding="latin-1") as file:
				content = file.read()
				srch = re.compile('Key ID\=\"(STR_ENH_[_a-zA-Z0-9]*)"', re.IGNORECASE)
				modStrings = srch.findall(content)
		modStrings = [s.lower() for s in modStrings]
		return modStrings

def getStringUsage(filepath):
		selfmodule = (re.search('(addons)[\W]*([_a-zA-Z0-9]*)', filepath)).group(2)
		submodule = (re.search(f'(addons)[\W]*{selfmodule}[\W]*([_a-zA-Z0-9]*)', filepath)).group(2)
...
def main(argv):
		...
		if ("str_enh_tagging_name" in allUsedStrings): allUsedStrings.remove("str_enh_tagging_name") # Handle tagging macro
