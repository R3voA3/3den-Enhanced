...
ENH_fnc_VIM_resetStorage_main =
{
  ...
  //Get all items defined in vehicle's config and add them to the listbox
  {
    private _classes = "true" configClasses (configOf ENH_VIM_target >> _x);
    {
        ...
        [_ctrlInventory, _configName, "", "", "", _count, _configName] call ENH_fnc_VIM_lnbAdd;
    } forEach _classes;
  ...
  false call ENH_fnc_VIM_applyAttribute;
...
[
  localize "STR_ENH_VIM_RESET_TOOLTIP",
  "",
  [
    "Yes",
    {call ENH_fnc_VIM_resetStorage_main; nil}//Return nil so the dialog is closed
  ],
  [
    "No",
    {}
  ...
  uiNamespace getVariable "ENH_Display_VIM"
