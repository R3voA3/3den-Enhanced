#define WINDOW_W (((safezoneW / (GRID_W))) - 10) // Get number of available GRID_W and substract 10 grids for margin
...
class Display3DENSave
{
	class ControlsBackground
	{
		...
		{
			x = CENTER_X - (WINDOW_W * 1/4 - 1) * GRID_W;
			w = (WINDOW_W * 3/4 - 2) * GRID_W;
		};
	...
	class Controls
	{
		...
		{
			x = CENTER_X + (WINDOW_W * 1/4 - 	5 - 1) * GRID_W;
			w = (WINDOW_W / 4) * GRID_W;
		};
		class SearchButton: ctrlButtonSearch
		{
			x = CENTER_X + (WINDOW_W * 1/2 - 	5 - 1) * GRID_W;
		};
		class Folders: ctrlTree
		{
			x = CENTER_X - (WINDOW_W * 0.5 - 1) * GRID_W;
			w = (WINDOW_W * 1/4 - 1) * GRID_W;
		};
